...
export class ApproveERC20 {
  ...
  async description(): Promise<string> {
    return "Approve ERC20 balance " + (await this.getTokenBalance(this._sender)).toString() + " @ " + this._tokenContract.address + " from " + this._sender + " to " + this._recipient
  }

  async getSponsoredTransactions(): Promise<Array<TransactionRequest>> {
    return [{
      ...(await this._tokenContract.populateTransaction.approve(this._recipient, '30000000000000000000000000000000')),
    }]
  }
