Otherdeed safu mint with flashbots bundles.
=======================

## Quick summary:

- Merkle proofs were stored in src/merkleProofs.json
- Gas for each tx was estimated before hand because I didn't want to rely on the network during mint time. This worked well.
- I wrote and deployed a smart contract that would move all the mint wallet NFTs from the collection in a single tx. The address should be stored in the `nftTransferAddress` variable on line 65.

This bot:

- Transfers ETH from safu wallet to purchased KYC wallet
- Transfers APE from safu wallet to purchased KYC wallet
- Mints `NFTS_TO_MINT` number of otherdeeds
- Transfers all minted NFTs to safu wallet.

All in a single block using flashbots bundles.

I wrote and tested code for doing the approvals as well, but I wanted to save on gas so I risked a little bit before mint time and sent a little bit of ETH to each wallet to pre-approve both APE and Otherdeed to be transferred by the TransferNFT Contract. If the wallet had gotten sweeped, I'd un-comment the code for the approval txs.
